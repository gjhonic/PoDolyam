...
func (s *Store) Personal(ctx context.Context, id, participant string) (meetings.Personal, error) {
	m, err := s.Get(ctx, id)
	if err != nil {
		return meetings.Personal{}, err
	}
	if m.State == "draft" {
		return meetings.Personal{}, errors.New("сначала зафиксируйте расчёт")
	}
	remaining, exists := m.Remaining[participant]
	if !exists {
		return meetings.Personal{}, meetings.ErrNotFound
	}
	p := meetings.Personal{Title: m.Title, Date: m.Date, State: m.State, Remaining: remaining, Lines: []meetings.PersonalLine{}}
	...
	for _, line := range m.Calculation.Lines {
		for _, share := range line.Shares {
			if share.ParticipantID == participant {
				p.Lines = append(p.Lines, meetings.PersonalLine{Name: names[line.ItemID], Amount: share.Amount})
			}
		}
	}
	for _, total := range m.Calculation.Totals {
